...
// One combined M-Pesa STK push (Daraja) for the total shortfall across all challenges.
...
export const POST = withErrorHandling(async function POST(req: NextRequest) {
  ...
  for (const f of friends) {
    if (!f.phone) {
      valid.push({ phone: null, nickname: f.nickname, stakePerPerson: f.stakePerPerson, userId: null, userName: null, userPhone: null });
      continue;
    }
    ...
    valid.push({ phone: f.phone, nickname: f.nickname, stakePerPerson: f.stakePerPerson, userId: found.id, userName: found.name, userPhone: found.phone });
  }

  if (!valid.length) {
    return NextResponse.json({ error: 'No valid friends to challenge.', skipped }, { status: 400 });
  }
  ...
  // Pre-generate access codes BEFORE the transaction (Supabase free tier: 1 connection)
    ...
    for (let i = 0; i < allocations.length; i++) {
      ...
      const ch = await tx.marketChallenge.create({
        data: {
          ...
          accessCode,
          userAId:          user.id,
          userBId:          alloc.userId || null,
          refereeId,
          stakePerPerson:   alloc.stakePerPerson,
          totalPool:        alloc.walletUsed,
  ...
  // ── Single Daraja STK push for combined shortfall ─────────────────────────
  ...
  if (totalMpesa > 0) {
    ...
    try {
      ...
      const stkResult = await stkPush({
        ...
        transactionDesc:  'CheckRada Batch',
      });
      ...
      await prisma.transaction.update({
        ...
        console.error(
          `[Batch] CRITICAL: mpesaRef update failed tx=${pending.id} ` +
          `accountRef=${accountRef} CheckoutRequestID=${stkResult.CheckoutRequestID} err=${err.message}`
        );
      });

      stkMessage = stkResult.CustomerMessage || `Check your phone for an M-Pesa prompt — KES ${totalMpesa}.`;
  ...
  // Notify friends whose challenges are immediately PENDING_JOIN
  const creatorName = displayName(user.name, user.phone);
  for (let i = 0; i < allocations.length; i++) {
    const alloc = allocations[i];
    const ch    = createdChallenges[i];
    if (!ch) continue;
    if (ch.status !== 'PENDING_JOIN') continue;
